...
def inference(args):
    ...
    for index, instance in enumerate(needle_dataset):
        ...
        needle_embedding_list.append(torch.load(args.embedding_by_name_dir + f"/{instance['path'].split('.')[0]}.pt", map_location="cpu").to(torch.bfloat16).to(accelerator.device))
        needle_embedding_interrupt_list.append(torch.load(args.embedding_by_name_dir + f"/{instance['interrupt_path'].split('.')[0]}.pt", map_location="cpu").to(torch.bfloat16).to(accelerator.device))
    ...
    for num_frames in tqdm(
        ...
        for depth in np.arange(0, 1 + args.depth_interval, args.depth_interval):
            ...
            for question_embedding, needle_embedding, needle_embedding_interrupt, answer_embedding, answer_id in zip(question_embeding_list, needle_embedding_list, needle_embedding_interrupt_list, answer_embedding_list, answer_id_list):
                ...
                # import pdb; pdb.set_trace()
                #! TODO: interrupt every 200 frames
                import random
...
if __name__ == "__main__":
    ...
    # embedding_by_name_dir
    args.add_argument("--embedding_by_name_dir", type=str, default=None)
