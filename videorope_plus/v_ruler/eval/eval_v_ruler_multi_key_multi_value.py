import sys
sys.path.append("/fs-computility/mllm/shared/weixilin")
import argparse
import gc
import sys
import torch
...
def get_position_ids(input_embeds, rope_type, video_se):
    ...
    elif rope_type == 'm_modify_rope':
        return get_m_modify_index(input_embeds, video_se)
    elif rope_type == 't_only':
        return get_m_rope_index(input_embeds, video_se)
    elif rope_type == 'pos_only':
        return get_m_modify_index(input_embeds, video_se)
    elif rope_type == 'm_modify_margin_rope':
        return get_m_modify_margin_index(input_embeds, video_se)
    elif rope_type == 'm_modify_no_center_rope':
        return get_m_modify_no_center_index(input_embeds, video_se)
    elif rope_type == 't_scale2_change_freq':
        scale_factor = 2.0
...
def inference(args):
    ...
    for index, instance in enumerate(needle_dataset):
        ...
        needle_lt = []
        for path in eval(instance["path"]):
            needle_lt.append(torch.load(args.needle_embedding_dir + f"/{path.split('.')[0]}.pt", map_location="cpu").to(torch.bfloat16).to(accelerator.device))
        needle_embedding_list.append(needle_lt)
    ...
    for num_frames in tqdm(
        ...
        for depth in np.arange(0, 1, args.depth_interval):
            accuracies = []
            for idx, (question_embedding, answer_embedding, answer_id) in enumerate(zip(question_embeding_list, answer_embedding_list, answer_id_list)):

                needle_list = needle_embedding_list[idx]
                num_needles = len(needle_list)

                # 根据 depth 计算插入帧的起止范围
                start_frame_idx = int(depth * num_frames)
                end_frame_idx = int((depth + args.depth_interval) * num_frames)
                end_frame_idx = min(end_frame_idx, num_frames)  # 避免越界

                insert_range = end_frame_idx - start_frame_idx
                if insert_range <= 0:
                    continue

                insert_indices = sorted([
                    random.randint(start_frame_idx, end_frame_idx - 1)
                ...
                input_chunks = []
                start_frame = 0
                for insert_idx, needle_embedding in zip(insert_indices, needle_list):
                    # 取 start_frame 到 insert_idx 之间的 haystack embeddings
                    chunk = haystack_embeddings[start_frame * IMAGE_TOKENS : insert_idx * IMAGE_TOKENS].to(accelerator.device)
                    input_chunks.append(chunk)
                    input_chunks.append(needle_embedding.to(accelerator.device))
                    start_frame = insert_idx

                # 添加剩下的 haystack_embeddings
                if start_frame < num_frames:
                    chunk = haystack_embeddings[start_frame * IMAGE_TOKENS : num_frames * IMAGE_TOKENS].to(accelerator.device)
                    input_chunks.append(chunk)

                # 拼接所有片段，形成最终 input_frames
                input_frames = torch.cat(input_chunks, dim=0).view(-1, haystack_embeddings.shape[-1]).unsqueeze(0)
...
def plot(args,  all_accuries):
    ...
    # 计算每个题目的平均准确率，根据all_accuries
    sum_acc_for_each_question = {}
    for instance in all_accuries:
        for idx, score in enumerate(instance["score_list"]):
            sum_acc_for_each_question[idx] = sum_acc_for_each_question.get(idx, 0) + score
    # 保留2位小数
    avg_acc_for_each_question = {k: round(v / len(all_accuries), 2) for k, v in sum_acc_for_each_question.items()}
    # save as txt
    with open(f"{args.output_path}/{model_name}/avg_accuracy.txt", "w") as f:
        for idx, acc in avg_acc_for_each_question.items():
            f.write(f"Question {idx}: {acc:.2f}\t num: {len(all_accuries)}\n")
        f.write(f"Average Accuracy: {average_accuracy:.2f}\n")
