...
def inference(args):
    ...
    for index, instance in enumerate(needle_dataset):
        ...
        for p in instance['path']:
            needle_embedding_tp.append(torch.load(os.path.join(args.needle_embedding_dir, f"{p.split('/')[-1].split('.')[0]}.pt"), map_location="cpu").to(torch.bfloat16).to(accelerator.device))
        needle_embedding_list.append(needle_embedding_tp)
    ...
    for num_frames in tqdm(
        ...
        for depth in np.arange(0, 1 + args.depth_interval, args.depth_interval):
            ...
            for question_embedding, needle_embedding, answer_embedding, answer_id in zip(question_embeding_list, needle_embedding_list, answer_embedding_list, answer_id_list):
                ...
                choose_indices = np.linspace(0, num_frames-1, len(needle_embedding), dtype=int)
                input_frames = []
                idx = 0
                for i in range(num_frames):
                    start_idx = i * IMAGE_TOKENS
                    end_idx = (i + 1) * IMAGE_TOKENS
                    input_frames.append(haystack_embeddings[start_idx:end_idx].to(accelerator.device))

                    if i in choose_indices:
                        input_frames.append(needle_embedding[idx].to(accelerator.device))
                        idx += 1

                # 在 query_frame_idx 处插入 needle_embedding
                # input_frames.insert(query_frame_idx, needle_embedding.to(accelerator.device))

                # 将 input_frames 拼接为一个张量
                input_frames = torch.cat(input_frames, dim=0).view(-1, haystack_embeddings.shape[-1]).unsqueeze(0)
...
if __name__ == "__main__":
    ...
    args.add_argument("--needle_dataset", type=str, default="/mnt/petrelfs/weixilin/projects/MLLM/Qwen2-VL/vision_niah/needle_datasets/dataset_order_frames.json")
    args.add_argument("--min_frame_num", type=int, default=200)
    args.add_argument("--frame_interval", type=int, default=100)
    args.add_argument("--output_path", type=str, default="/mnt/petrelfs/weixilin/projects/MLLM/Qwen2-VL/vision_niah/niah_output_order_frames")
    ...
    args.add_argument("--haystack_dir", type=str, default="/mnt/petrelfs/weixilin/projects/MLLM/Qwen2-VL/vision_niah/video_needle_haystack/data/haystack_vicuna_embeddings_6000frames")
    args.add_argument("--needle_embedding_dir", type=str, default="/mnt/petrelfs/weixilin/projects/MLLM/Qwen2-VL/vision_niah/video_needle_haystack/data/needle_vicuna_embeddings_by_name")
