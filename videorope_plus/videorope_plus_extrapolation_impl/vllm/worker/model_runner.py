...
class ModelInputForGPUBuilder(ModelRunnerInputBuilderBase[ModelInputForGPU]):
    ...
    def _compute_lens(self, inter_data: InterDataForSeqGroup, seq_idx: int,
        ...
        if hasattr(self.runner, 'which_rope'):
            self.which_rope = self.runner.which_rope
            self.scale_factor = self.runner.scale_factor
            self.scaling_method = self.runner.scaling_method
            self.exp_factor = self.runner.exp_factor
        elif self.runner.vllm_config.additional_config is not None and 'which_rope' in self.runner.vllm_config.additional_config:
            self.which_rope = self.runner.vllm_config.additional_config['which_rope']
            self.scale_factor = self.runner.vllm_config.additional_config['scale_factor']
            self.scaling_method = self.runner.vllm_config.additional_config['scaling_method']
            self.exp_factor = self.runner.vllm_config.additional_config['exp_factor']

            self.runner.which_rope = self.which_rope
            self.runner.scale_factor = self.scale_factor
            self.runner.scaling_method = self.scaling_method
            self.runner.exp_factor = self.exp_factor
            MRotaryEmbedding.scaling_method = self.scaling_method
            MRotaryEmbedding.factor = self.exp_factor

        else:
            self.which_rope = 'm_rope'
            self.scale_factor = 1.0
            self.scaling_method = 'default'
            self.exp_factor = 4.0
            
        print(self.which_rope)
    ...
    def _compute_multi_modal_input(self, inter_data: InterDataForSeqGroup,
        ...
        if self.runner.model_config.uses_mrope:
            ...
            for seq_idx in range(inter_data.n_seqs):
                ...
                if self.which_rope == 'm_rope':
                ...
                elif self.which_rope == 'time_rope':
                    mrope_input_positions, mrope_position_delta = \
                            MRotaryEmbedding.get_input_time_positions(
                                token_ids,
                                hf_config=hf_config,
                                image_grid_thw=image_grid_thw,
                                video_grid_thw=video_grid_thw,
                    ...
                    mrope_input_positions_origin, _ = \
                            MRotaryEmbedding.get_input_vanilla_positions(
                                token_ids,
                                hf_config=hf_config,
                                image_grid_thw=image_grid_thw,
                                video_grid_thw=video_grid_thw,
                    ...
                    seq_data.mrope_position_delta = mrope_position_delta
                    inter_data.mrope_input_positions[
                        seq_idx] = np.add(mrope_input_positions, mrope_input_positions_origin).tolist()
                    ...
                    inter_data.mrope_input_positions[
                        seq_idx] = mrope_input_positions
                elif self.which_rope == 't_scale2_change_freq':
                    mrope_input_positions, mrope_position_delta = \
                            MRotaryEmbedding.get_input_t_scale_modify_positions(
                                token_ids,
                                hf_config=hf_config,
                                image_grid_thw=image_grid_thw,
                                video_grid_thw=video_grid_thw,
                    ...
                    MRotaryEmbedding.forward = MRotaryEmbedding.m_modify_forward
                    seq_data.mrope_position_delta = mrope_position_delta
                    inter_data.mrope_input_positions[
                        seq_idx] = mrope_input_positions
                elif self.which_rope == 'ttxy_rope':
                    mrope_input_positions, mrope_position_delta = \
                            MRotaryEmbedding.get_input_t_scale_modify_positions(
                                token_ids,
                                hf_config=hf_config,
                    ...
                    MRotaryEmbedding.forward = MRotaryEmbedding.ttxy_modify_forward
                    seq_data.mrope_position_delta = mrope_position_delta
                    inter_data.mrope_input_positions[
                        seq_idx] = mrope_input_positions
                else:
                    raise ValueError(f"not implement {self.which_rope}")
